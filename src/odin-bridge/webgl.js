...
export class WebGLInterface {
  ...
  getSource(shader, strings_ptr, strings_length) {
    ...
    for (let i = 0; i < strings_length; i++) {
      let ptr = this.wmi.loadPtr(strings_ptr + i * sizeof_string);
      let len = this.wmi.loadInt(strings_ptr + i * sizeof_string + sizeof_ptr + padding);
      let str = this.wmi.loadString(ptr, len);
      source += str;
    }
    return source;
  }
    ...
    return {
      ...
      GetWebGLVersion: (major_ptr, minor_ptr) => {
        ...
        if (version.indexOf("WebGL 2.0") !== -1) {
          this.wmi.storeI32(major_ptr, 2);
          this.wmi.storeI32(minor_ptr, 0);
          return;
        }

        this.wmi.storeI32(major_ptr, 1);
        this.wmi.storeI32(minor_ptr, 0);
      },
      GetESVersion: (major_ptr, minor_ptr) => {
        let version = this.ctx.getParameter(0x1F02);
        if (version.indexOf("OpenGL ES 3.0") !== -1) {
          this.wmi.storeI32(major_ptr, 3);
          this.wmi.storeI32(minor_ptr, 0);
          return;
        }
      ...
      BufferData: (target, size, data, usage) => {
        if (data) {
          this.ctx.bufferData(target, this.wmi.loadBytes(data, size), usage);
        } else {
      ...
      BufferSubData: (target, offset, size, data) => {
        if (data) {
          this.ctx.bufferSubData(target, offset, this.wmi.loadBytes(data, size));
        } else {
      ...
      CompressedTexImage2D: (target, level, internalformat, width, height, border, imageSize, data) => {
        if (data) {
          this.ctx.compressedTexImage2D(target, level, internalformat, width, height, border, this.wmi.loadBytes(data, imageSize));
        } else {
      ...
      CompressedTexSubImage2D: (target, level, xoffset, yoffset, width, height, format, imageSize, data) => {
        if (data) {
          this.ctx.compressedTexSubImage2D(target, level, xoffset, yoffset, width, height, format, this.wmi.loadBytes(data, imageSize));
        } else {
      ...
      DrawElements: (mode, count, type, indices) => {
        // TODO: Push patch with corrected types. (count is GLsizei which is i32)
        // Remove Number cast
      ...
      GetParameter4i: (pname, v0, v1, v2, v3) => {
        ...
        this.wmi.storeI32(v0, i4[0]);
        this.wmi.storeI32(v1, i4[1]);
        this.wmi.storeI32(v2, i4[2]);
        this.wmi.storeI32(v3, i4[3]);
      },
      ...
      GetProgramInfoLog: (program, buf_ptr, buf_len, length_ptr) => {
        ...
        if (buf_len > 0 && buf_ptr) {
          ...
          this.wmi.loadBytes(buf_ptr, buf_len).set(new TextEncoder().encode(log))

          this.wmi.storeInt(length_ptr, n);
        }
      },
      ...
      ReadnPixels: (x, y, width, height, format, type, bufSize, data) => {
        this.ctx.readPixels(x, y, width, height, format, type, this.wmi.loadBytes(data, bufSize));
      },
      ...
      TexImage2D: (target, level, internalformat, width, height, border, format, type, size, data) => {
        if (data) {
          this.ctx.texImage2D(target, level, internalformat, width, height, border, format, type, this.wmi.loadBytes(data, size));
        } else {
      ...
      TexSubImage2D: (target, level, xoffset, yoffset, width, height, format, type, size, data) => {
        this.ctx.texSubImage2D(target, level, xoffset, yoffset, width, height, format, type, this.wmi.loadBytes(data, size));
      },
  ...
  getWebGL2Interface() {
    return {
      ...
      GetBufferSubData: (target, srcByteOffset, dst_buffer_ptr, dst_buffer_len, dstOffset, length) => {
        this.assertWebGL2();
        this.ctx.getBufferSubData(target, srcByteOffset, this.wmi.loadBytes(dst_buffer_ptr, dst_buffer_len), dstOffset, length);
      },
      ...
      InvalidateFramebuffer: (target, attachments_ptr, attachments_len) => {
        this.assertWebGL2();
        let attachments = this.wmi.loadU32Array(attachments_ptr, attachments_len);
      ...
      TexImage3D: (target, level, internalformat, width, height, depth, border, format, type, size, data) => {
        this.assertWebGL2();
        if (data) {
          this.ctx.texImage3D(target, level, internalformat, width, height, depth, border, format, type, this.wmi.loadBytes(data, size));
        } else {
      ...
      TexSubImage3D: (target, level, xoffset, yoffset, zoffset, width, height, depth, format, type, size, data) => {
        this.assertWebGL2();
        this.ctx.texSubImage3D(target, level, xoffset, yoffset, zoffset, width, height, depth, format, type, this.wmi.loadBytes(data, size));
      },
      CompressedTexImage3D: (target, level, internalformat, width, height, depth, border, imageSize, data) => {
        this.assertWebGL2();
        if (data) {
          this.ctx.compressedTexImage3D(target, level, internalformat, width, height, depth, border, this.wmi.loadBytes(data, imageSize));
        } else {
      ...
      CompressedTexSubImage3D: (target, level, xoffset, yoffset, zoffset, width, height, depth, format, imageSize, data) => {
        this.assertWebGL2();
        if (data) {
          this.ctx.compressedTexSubImage3D(target, level, xoffset, yoffset, zoffset, width, height, depth, format, this.wmi.loadBytes(data, imageSize));
        } else {
      ...
      GetFragDataLocation: (program, name_ptr, name_len) => {
        this.assertWebGL2();
        return this.ctx.getFragDataLocation(this.programs[program], this.wmi.loadString(name_ptr, name_len));
      },
      ...
      TransformFeedbackVaryings: (program, varyings_ptr, varyings_len, bufferMode) => {
        this.assertWebGL2();
        const stringSize = this.wmi.intSize * 2;
        let varyings = [];
        for (let i = 0; i < varyings_len; i++) {
          let ptr = this.wmi.loadPtr(varyings_ptr + i * stringSize + 0 * 4);
          let len = this.wmi.loadPtr(varyings_ptr + i * stringSize + 1 * 4);
          varyings.push(this.wmi.loadString(ptr, len));
        }
      ...
      GetUniformBlockIndex: (program, uniformBlockName_ptr, uniformBlockName_len) => {
        this.assertWebGL2();
        return this.ctx.getUniformBlockIndex(this.programs[program], this.wmi.loadString(uniformBlockName_ptr, uniformBlockName_len));
      },
      ...
      GetActiveUniformBlockName: (program, uniformBlockIndex, buf_ptr, buf_len, length_ptr) => {
        ...
        this.wmi.loadBytes(buf_ptr, buf_len).set(new TextEncoder().encode(name))
        this.wmi.storeInt(length_ptr, n);
      },
